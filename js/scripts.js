...
function cleanInputs() {
  ...
  imcInfo.classList.remove("low", "good", "medium", "high");
}
...
calcBtn.addEventListener("click", (e) => {
  ...
  const infoItem = data.find(item => imc >= item.min && imc <= item.max);

  if (!infoItem) return;

  imcNumber.innerText = imc;
  imcInfo.innerText = infoItem.info;

  imcNumber.classList.remove("low", "good", "medium", "high");
  imcInfo.classList.remove("low", "good", "medium", "high");

  switch (infoItem.info) {
    case "Magreza":
